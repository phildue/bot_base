...
/// (www.mbedded.ninja) \created		2014-01-07 \last-modified
/// 2019-05-30 \brief			The main serial port class. \details
///					See README.rst in repo root dir for more
/// info.
...
/// \brief		Strongly-typed enumeration of baud rates for use with
/// the SerialPort class \details    Specifies all the same baud rates as UNIX,
/// as well as B_CUSTOM to specify your
...
/// \brief		SerialPort object is used to perform rx/tx serial
/// communication.
class SerialPort {

public:
  /// \brief		Default constructor. You must specify at least the
  /// device before calling Open().
  SerialPort();
  ...
  /// \details    Only call when state != OPEN. This method manupulates VMIN and
  /// VTIME. \param      timeout_ms  Set to -1 to infinite timeout, 0 to return
  /// immediately with any data (non
  ///             blocking, or >0 to wait for data for a specified number of
  ///             milliseconds). Timeout will be rounded to the nearest 100ms (a
  ///             Linux API restriction). Maximum value limited to 25500ms
  ///             (another Linux API restriction).
  void SetTimeout(int32_t timeout_ms);

  /// \brief		Enables/disables echo.
  /// \param		value		Pass in true to enable echo, false to
  /// disable echo.
  ...
  /// \param		data		The data that will be written to the COM
  /// port. \throws		CppLinuxSerial::Exception if state != OPEN.
  void Write(const std::string &data);

  /// \brief		Use to read from the COM port.
  /// \param		data		The object the read characters from the
  /// COM port will be saved to. \param      wait_ms     The amount of time to
...
private:
  ...
  /// \brief		Returns a populated termios2 structure for the serial
  /// port pointed to by the file descriptor.
  ...
  /// \brief		The file descriptor for the open file. This gets written
  /// to when Open() is called.
  int fileDesc_;
