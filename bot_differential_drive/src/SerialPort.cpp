...
void SerialPort::Open() {
  ...
  if (device_.empty()) {
    THROW_EXCEPT(
        "Attempted to open file when file path has not been assigned to.");
  }

  // Attempt to open file
  // this->fileDesc = open(this->filePath, O_RDWR | O_NOCTTY | O_NDELAY);

  // O_RDONLY for read-only, O_WRONLY for write only, O_RDWR for both read/write
  // access 3rd, optional parameter is mode_t mode
...
void SerialPort::ConfigureTermios() {
  ...
  if (baudRateType_ == BaudRateType::STANDARD) {
    ...
    case BaudRate::B_460800:
      ...
      break;
    default:
      throw std::runtime_error(std::string() + "baudRate passed to " +
                               __PRETTY_FUNCTION__ + " unrecognized.");
    }
  }
  ...
  // them separate for now....
    ...
    // baudRateCustom_ * 102 / 100) { 	printf("Cannot set serial port speed to
    // %d. Closest possible is %d\n", baudRateCustom_, closestSpeed);
    // }
  ...
  // VMIN = 0, VTIME > 0: Block until any amount of data is available, OR
  // timeout occurs VMIN > 0, VTIME > 0: Block until either VMIN characters have
  // been received, or VTIME
  //                      after first character has elapsed
  // c_cc[WMIN] sets the number of characters to block (wait) for when read() is
  // called. Set to 0 if you don't want read to block. Only meaningful when port
...
void SerialPort::Read(std::string &data) {
  ...
  if (fileDesc_ == 0) {
    ...
    // return false;
    THROW_EXCEPT("Read() was called but file descriptor (fileDesc) was 0, "
...
// 	{
// 		// Error occurred
// 		std::cout << "Could not get terminal attributes for \"" <<
// device_
// << "\" - " << strerror(errno) << std::endl; 		throw
...
// 	{
// 		// Error occurred
// 		std::cout << "Could not apply terminal attributes for \"" <<
// device_ << "\" - " << strerror(errno) << std::endl; 		throw
// std::system_error(EFAULT, std::system_category());
...
void SerialPort::Close() {
  if (fileDesc_ != -1) {
    ...
    if (retVal != 0)
      THROW_EXCEPT("Tried to close serial port " + device_ +
                   ", but close() failed.");
...
void SerialPort::SetTimeout(int32_t timeout_ms) {
  ...
  if (timeout_ms > 25500)
    THROW_EXCEPT(std::string() + "timeout_ms provided to " +
                 __PRETTY_FUNCTION__ + " was > 25500, which is invalid.");
  if (state_ == State::OPEN)
    THROW_EXCEPT(std::string() + __PRETTY_FUNCTION__ +
